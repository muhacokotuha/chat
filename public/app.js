    ...
    fetch('/messages').then((response) => {
       ...
       if(messages.length !== 0){
           messages.forEach(element => {
               ...
               switch (element.type){
               case "msg":
               li.innerHTML = '<span class="' + element.color +'">' + element.name+"</span> : "+element.content
               break
               case "image":
               li.innerHTML = '<span class="' + element.color +'">' + element.name+"</span> : <img src='/images/"+element.content + "' /img>"
               }
